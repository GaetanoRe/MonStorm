...
Hit Applier object (the weapon that will do the damage)
 - Add HitApplierComponentConfigured or HitApplierComponentStandalone script and trigger box collider component, and set the Damage variable in the inspector.
...
Hittable Object (the object that will get hit and take damage)
 - Hittable Object (parent), add a script that inherits from IHitReceiver interface and Rigidbody
 - Add child objects for each separate body component that needs to take different amount of damage (head, arms, etc.),
   add HitDetectorComponentConfigured or HitDetectorComponentStandalone script to each and set the Damage Multiplier variable, also add a trigger box collider component
